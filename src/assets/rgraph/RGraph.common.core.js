...
(function (win, doc, undefined) {
  ...
  RGraph.getScale = function (args) {
    var properties = args.object.properties,
      numlabels =
        typeof args.options['scale.labels.count'] == 'number'
          ? args.options['scale.labels.count']
          : 5,
      units_pre =
        typeof args.options['scale.units.pre'] == 'string' ? args.options['scale.units.pre'] : '',
      units_post =
        typeof args.options['scale.units.post'] == 'string' ? args.options['scale.units.post'] : '',
    ...
    if (!max) {
      ...
      var arr = [
          1, 0.5, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001, 0.00005, 0.00001, 0.000005,
          0.000001, 0.0000005, 0.0000001, 0.00000005, 0.00000001, 0.000000005, 0.000000001,
          0.0000000005, 0.0000000001, 0.00000000005, 0.00000000001, 0.000000000005, 0.000000000001,
          0.0000000000005
        ],
        vals = [];
      ...
      var interval = Math.pow(
        10,
        Math.max(1, Number(String(Number(max) - Number(min)).length - 1))
      );
  ...
  RGraph.parseJSONGradient = function (args) {
    ...
    if (typeof def.r1 === 'number' && typeof def.r2 === 'number') {
      ...
      var grad = context.createLinearGradient(def.x1, def.y1, def.x2, def.y2);
    }
  ...
  //
  RGraph.arrayFill = RGraph.arrayPad = function () {
  ...
  RGraph.clear = RGraph.Clear = function (args) {
    ...
    if (!color || (color && color === 'rgba(0,0,0,0)') || color === 'transparent') {
      ...
      obj.path('fs % fr -100 -100 % %', color, args.canvas.width + 200, args.canvas.height + 200);
    } else {
      obj.path(
        'fs % fr -100 -100 % %',
        obj.get('clearto'),
  ...
  RGraph.drawTitle = function () {
    var args = RGraph.getArgs(arguments, 'object'),
      ...
      valign = 'center',
      x =
        (obj.canvas.width - obj.properties.marginLeft - obj.properties.marginRight) / 2 +
    ...
    if (
      obj.properties.key &&
      obj.properties.key.length &&
      obj.properties.keyPosition &&
      obj.properties.keyPosition !== 'margin'
    ) {
      var valign = 'center';
    } else if (
      obj.properties.key &&
  ...
  RGraph.getMouseXY = function () {
    ...
    // them
    if (typeof window.event.offsetX === 'number' && typeof window.event.offsetY === 'number') {
  ...
  RGraph.getCanvasXY = function () {
    ...
    var borderLeft = args.canvas.style.borderLeftWidth
      ? parseInt(args.canvas.style.borderLeftWidth)
      : 0;
    var borderTop = args.canvas.style.borderTopWidth
      ? parseInt(args.canvas.style.borderTopWidth)
      : 0;
  ...
  RGraph.Background.draw = function () {
    ...
    if (
      ...
      if (typeof properties.yaxisTitlePos == 'number') {
        ...
        if (args.object && args.object.scale2) {
          var yaxisTitleDimensions = RGraph.measureText({
            ...
            bold:
              typeof properties.yaxisScaleBold === 'boolean'
                ? properties.yaxisScaleBold
                : properties.textBold,
            font: properties.yaxisScaleFont || properties.textFont,
            size:
              typeof properties.yaxisScaleSize === 'number'
                ? properties.yaxisScaleSize
                : properties.textSize
          });
        } else {
      ...
      if (
        properties.yaxisTitleAlign == 'right' ||
        properties.yaxisTitlePosition == 'right' ||
        (args.object.type === 'hbar' &&
          properties.yaxisPosition === 'right' &&
          typeof properties.yaxisTitleAlign === 'undefined' &&
          typeof properties.yaxisTitlePosition === 'undefined')
      ) {
        angle = 90;
        yaxis_title_pos =
          typeof properties.yaxisTitlePos === 'number'
            ? args.object.canvas.width - marginRight + properties.yaxisTitlePos * marginRight
            : args.object.canvas.width -
              marginRight +
              (properties.yaxisLabelsSize || properties.textSize) +
              10;
      }
    ...
    if (bgcolor) {
      ...
      args.object.context.fillRect(
        ...
        args.object.canvas.height - marginTop - marginBottom
      );
    }
    ...
    var func = function (obj, cacheCanvas, cacheContext) {
      ...
      if (properties.backgroundGrid) {
        ...
        if (properties.backgroundGridAutofit) {
          ...
          if (properties.backgroundGridAutofitAlign) {
            ...
            if (obj.type === 'line') {
              ...
              obj.set('backgroundGridVlinesCount', obj.data.length + (properties.total ? 1 : 0));
          ...
          var vsize =
            (cacheCanvas.width - marginLeft - marginRight) / properties.backgroundGridVlinesCount;
          var hsize =
            (cacheCanvas.height - marginTop - marginBottom) / properties.backgroundGridHlinesCount;
        ...
        obj.context.beginPath();
        cacheContext.lineWidth = properties.backgroundGridLinewidth
        ...
        if (properties.backgroundGridHlines) {
          ...
          for (y = marginTop; y <= height + 1; y += hsize) {
            ...
            if (
              args.object.properties.variant === '3d' &&
              args.object.type === 'bar' &&
        ...
        if (properties.backgroundGridBorder) {
          ...
          cacheContext.strokeRect(
            Math.round(marginLeft),
            Math.round(marginTop),
            obj.canvas.width - marginLeft - marginRight,
            obj.canvas.height - marginTop - marginBottom
          );
        }
      }
    ...
    if (properties.backgroundBorder) {
      var color = RGraph.isString(properties.backgroundBorderColor)
        ? properties.backgroundBorderColor
        : '#aaa';
      var linewidth = RGraph.isNumber(properties.backgroundBorderLinewidth)
        ? properties.backgroundBorderLinewidth
        : 1;

      // Dashed background border
      if (
        properties.backgroundBorderDashed &&
        typeof args.object.context.setLineDash == 'function'
      ) {
      ...
      // Dotted background grid
      if (
        properties.backgroundBorderDotted &&
        typeof args.object.context.setLineDash == 'function'
      ) {
      ...
      args.object.path(
        ...
        args.object.canvas.width -
          args.object.properties.marginLeft -
          args.object.properties.marginRight,
        args.object.canvas.height -
          args.object.properties.marginTop -
          args.object.properties.marginBottom,
        color
      );
  ...
  RGraph.drawBars = function () {
    ...
    for (var i = 0, len = hbars.length; i < len; ++i) {
      ...
      if (start + length < -1 * args.object.scale2.max)
        length = -1 * args.object.scale2.max - start;

      if (
        properties.xaxisPosition == 'center' &&
        start == args.object.scale2.max &&
      ...
      for (var i = 0, len = labels_processed.length; i < len; i += 1) {
        if (labels_processed[i]) {
          ...
          if (coords && coords.length > 0) {
            var x =
              (args.object.type == 'bar' ? coords[0] + coords[2] / 2 : coords[0]) +
              (properties.labelsIngraphOffsetx || 0);
            var y =
              (args.object.type == 'bar' ? coords[1] + coords[3] / 2 : coords[1]) +
              (properties.labelsIngraphOffsety || 0);
            ...
            if (args.object.type === 'bar') {
              ...
              if (
                ...
                if (
                  text_y < 5 &&
                  (typeof labels_processed[i] === 'string' ||
                    typeof labels_processed[i][3] === 'undefined')
                ) {
                  text_y = y + 5 + length;
                  var valign = 'top';
                }
            ...
            if (
              typeof labels_processed[i] === 'object' &&
              typeof labels_processed[i][1] === 'string'
            ) {
            ...
            RGraph.text({
              ...
              text:
                typeof labels_processed[i] === 'object' &&
                typeof labels_processed[i][0] === 'string'
              ...
              'bounding.fill':
                typeof labels_processed[i] === 'object' &&
                typeof labels_processed[i][2] === 'string'
    ...
    if (xaxis) {
      if (xaxispos === 'center') {
        args.object.path(
          'b m % % l % % l % % l % % c s #aaa f %',
          marginLeft,
          marginTop + halfGraphArea,
          marginLeft + offsetx,
          marginTop + halfGraphArea - offsety,
          args.object.canvas.width - marginRight + offsetx,
          marginTop + halfGraphArea - offsety,
          args.object.canvas.width - marginRight,
          marginTop + halfGraphArea,
  ...
  RGraph.draw3DYAxis = function (args) {
    ...
    if (
      (args.object.type === 'hbar' || args.object.type === 'bar') &&
      properties.yaxisPosition === 'center'
    ) {
    ...
    args.object.path(
      ...
      x,
      args.object.canvas.height - marginBottom,
  ...
  RGraph.roundedRect = function () {
    var args = RGraph.getArgs(
      arguments,
      'context,x,y,width,height,radius,roundtl,roundtr,roundbl,roundbr'
    );
  ...
  RGraph.addCustomEventListener = function () {
    ...
    RGraph.events[args.object.uid].push([args.object, args.name, args.func]);
    ...
    if (typeof properties.backgroundImage === 'string') {
      ...
      if (typeof align === 'string') {
        if (align.indexOf('right') != -1) {
          var x =
            args.object.canvas.width - (properties.backgroundImageW || img.width) - marginRight;
        } else {
          var x = marginLeft;
        }
        ...
        if (align.indexOf('bottom') != -1) {
          var y =
            args.object.canvas.height - (properties.backgroundImageH || img.height) - marginBottom;
        } else {
          var y = marginTop;
        }
      } else {
  ...
  RGraph.ObjectRegistry.add = function () {
    ...
    //
    RGraph.ObjectRegistry.objects.byCanvasID.push([id, args.object]);
  };
  ...
  RGraph.getHypLength = function () {
    ...
    return Math.sqrt(
      (args.x2 - args.x1) * (args.x2 - args.x1) + (args.y2 - args.y1) * (args.y2 - args.y1)
    );
  };
  ...
  RGraph.installEventListeners = function () {
    ...
    if (RGraph.installCanvasClickListener) {
      ...
      RGraph.hasTooltips(args.object) ||
        ...
        if (RGraph.isNullish(args.object)) {
          ...
          for (var j in obj) {
            str +=
              ...
              j +
              ' => ' +
              RGraph.pr(args.object[j], true, indent + '    ', counter + 1) +
              '\n';
          }
          str += indent + '}';
        }
  ...
  //
  RGraph.AJAX.number = RGraph.AJAX.getNumber = function () {
    var args = RGraph.getArgs(arguments, 'url,callback');
  ...
  //
  RGraph.AJAX.string = RGraph.AJAX.getString = function () {
    var args = RGraph.getArgs(arguments, 'url,callback');
  ...
  RGraph.text = function (args) {
    ...
    if (
      arguments[0] &&
      arguments[1] &&
      (typeof arguments[1].text === 'string' || typeof arguments[1].text === 'number')
    ) {
      var obj = arguments[0],
        args = arguments[1];
    ...
    function domtext() {
      ...
      var cacheKey =
        Math.abs(parseInt(args.x)) +
        '_' +
        Math.abs(parseInt(args.y)) +
        '_' +
        String(args.text).replace(/[^a-zA-Z0-9]+/g, '_') +
        '_' +
        obj.canvas.id;
      ...
      if (!obj.canvas.rgraph_domtext_wrapper) {
        ...
        wrapper.style.overflow =
          obj.properties.textAccessibleOverflow != false &&
          obj.properties.textAccessibleOverflow != 'hidden'
            ? 'visible'
            : 'hidden';
      ...
      if (!RGraph.text.domNodeCache[obj.id] || !RGraph.text.domNodeCache[obj.id][cacheKey]) {
        ...
        //
        span.style.left =
          args.x * (parseInt(obj.canvas.offsetWidth) / parseInt(obj.canvas.width)) + 'px';
        span.style.top =
          args.y * (parseInt(obj.canvas.offsetHeight) / parseInt(obj.canvas.height)) + 'px';
        ...
        if (args.bounding) {
          ...
          span.style.borderWidth =
            typeof args['bounding.linewidth'] === 'number'
              ? args['bounding.linewidth']
              : defaults.bounding.linewidth;
        }
        // Pointer events
        if (
          (typeof obj.properties.textAccessiblePointerevents === 'undefined' ||
          ...
          for (i in RGraph.text.domNodeCache) {
            for (j in RGraph.text.domNodeCache[i]) {
              if (RGraph.text.domNodeCache[i][j] && RGraph.text.domNodeCache[i][j].parentNode) {
                RGraph.text.domNodeCache[i][j].parentNode.removeChild(
                  RGraph.text.domNodeCache[i][j]
                );
              }
            }
          }
      ...
      RGraph.text.find = function (args) {
        ...
        if (args.object && args.object.isrgraph) {
          ...
          alert(
            '[RGRAPH] You Must give either an object or an ID to the RGraph.text.find() function'
          );
          return false;
        }
    ...
    if (obj && obj.isrgraph) {
      ...
      obj.toString().indexOf('CanvasRenderingContext2D') != -1 ||
      (RGraph.ISIE8 && obj.moveTo)
    ) {
      var context = obj;
      var canvas = obj.canvas;
      var obj = canvas.__object__;
    ...
    if (typeof args.boundingLinewidth === 'number')
      args['bounding.linewidth'] = args.boundingLinewidth;
    ...
    var bounding = args.bounding,
      ...
      bounding_linewidth =
        typeof args['bounding.linewidth'] === 'number' ? args['bounding.linewidth'] : 1;
    ...
    //
    context.font =
      (args.italic ? 'italic ' : '') + (args.bold ? 'bold ' : '') + size + 'pt ' + font;
  ...
  RGraph.parseDate = function () {
    ...
    for (var i = 0, len = tokens.length; i < len; ++i) {
      if (tokens[i]) {
        ...
        if (
          tokens[i].match(new RegExp('^(\\d\\d\\d\\d)' + sep + '(\\d\\d)' + sep + '(\\d\\d)$', 'i'))
        ) {
        ...
        if (
          tokens[i].match(new RegExp('^(\\d\\d)' + sep + '(\\d\\d)' + sep + '(\\d\\d\\d\\d)$', 'i'))
        ) {
  ...
  RGraph.parseDateOld = function () {
    ...
    if (args.str.match(/^(\d\d)(?:-|\/)(\d\d)(?:-|\/)(\d\d\d\d)(.*)$/)) {
      args.str = '{1}/{2}/{3}{4}'.format(RegExp.$3, RegExp.$2, RegExp.$1, RegExp.$4);
    }
  ...
  RGraph.resetColorsToOriginalValues = function () {
    ...
    if (args.object.original_colors) {
      ...
      for (var j in args.object.original_colors) {
        if (typeof j === 'string') {
          // TAKEN OUT 1st AUGUST && j.substr(0,6) === 'chart.'
  ...
  RGraph.radialGradient = function () {
    ...
    for (var i = 0; i < args.colors.length; ++i) {
      gradient.addColorStop(i / (numColors - 1), args.colors[i]);
    }
  ...
  RGraph.arrayRandom = function () {
    ...
    for (var i = 0, arr = []; i < args.count; i += 1) {
      arr.push(RGraph.random(args.min, args.max, args.decimals));
    }
  ...
  RGraph.isArray = function (o) {
    ...
    return o != null && typeof pos === 'number' && pos > 0 && pos < 20;
  };
  ...
  RGraph.reset = function () {
    ...
    // already exist
    if (!RGraph.text.domNodeCache) {
      RGraph.text.domNodeCache = [];
    }
    if (!RGraph.text.domNodeDimensionCache) {
      RGraph.text.domNodeDimensionCache = [];
    }
  ...
  RGraph.stringsToNumbers = function () {
    ...
    if (typeof args.string === 'object' && !RGraph.isNullish(args.string)) {
      for (var i = 0, len = args.string.length; i < len; i += 1) {
        args.string[i] = RGraph.stringsToNumbers(args.string[i], args.separator);
      }
    }
  ...
  RGraph.cachedDraw = function () {
    ...
    if (!RGraph.cache[args.id]) {
      ...
      RGraph.cache[args.id].canvas.setAttribute(
        'id',
        'background_cached_canvas' + args.object.canvas.id
      );
    ...
    for (var i = 0, len = p.length; i < len; i += 1) {
      switch (p[i]) {
        ...
        case 'c':
          ...
          break;
        case 'm':
          context.moveTo(parseFloat(p[i + 1]), parseFloat(p[i + 2]));
          i += 2;
          break;
        case 'l':
          context.lineTo(parseFloat(p[i + 1]), parseFloat(p[i + 2]));
          i += 2;
          break;
        case 's':
          if (p[i + 1] && p[i + 1] !== 'null') context.strokeStyle = p[i + 1];
          context.stroke();
          i++;
          break;
        case 'f':
          if (p[i + 1] && p[i + 1] !== 'null') {
            context.fillStyle = p[i + 1];
          }
          context.fill();
          i++;
          break;
        ...
        case 'a':
          context.arc(
            ...
            parseFloat(p[i + 4]),
            parseFloat(p[i + 5]),
            p[i + 6] === 'true' || p[i + 6] === true || p[i + 6] === 1 || p[i + 6] === '1'
              ? true
              : false
          );
          i += 6;
        ...
        case 'at':
          ...
          break;
        case 'lw':
          context.lineWidth = parseFloat(p[i + 1]);
          i++;
          break;
        case 'e':
          context.ellipse(
          ...
          break;
        case 'lj':
          context.lineJoin = p[i + 1];
          i++;
          break;
        case 'lc':
          context.lineCap = p[i + 1];
          i++;
          break;
        case 'sc':
          context.shadowColor = p[i + 1];
          i++;
          break;
        case 'sb':
          context.shadowBlur = parseFloat(p[i + 1]);
          i++;
          break;
        case 'sx':
          context.shadowOffsetX = parseFloat(p[i + 1]);
          i++;
          break;
        case 'sy':
          context.shadowOffsetY = parseFloat(p[i + 1]);
          i++;
          break;
        case 'fs':
          context.fillStyle = p[i + 1];
        ...
        case 'rs':
          ...
          break;
        case 'tr':
          context.translate(parseFloat(p[i + 1]), parseFloat(p[i + 2]));
          i += 2;
          break;
        case 'sl':
          context.scale(parseFloat(p[i + 1]), parseFloat(p[i + 2]));
          i += 2;
          break;
        case 'ro':
          context.rotate(parseFloat(p[i + 1]));
          i++;
          break;
        case 'tf':
          context.transform(
        ...
        case 'cr':
          ...
          break;
        case 'ld':
          var parts = eval(p[i + 1]);
          context.setLineDash(parts);
          i += 1;
          break;
        case 'ldo':
          context.lineDashOffset = p[i + 1];
          i++;
          break;
        case 'fo':
          context.font = p[i + 1];
          ...
          break;
        case 'ft':
          context.fillText(p[i + 1], parseFloat(p[i + 2]), parseFloat(p[i + 3]));
          i += 3;
          break;
        case 'st':
          context.strokeText(p[i + 1], parseFloat(p[i + 2]), parseFloat(p[i + 3]));
          i += 3;
          break;
        case 'ta':
          context.textAlign = p[i + 1];
          i++;
          break;
        case 'tbl':
          context.textBaseline = p[i + 1];
          i++;
          break;
        case 'ga':
          context.globalAlpha = parseFloat(p[i + 1]);
          i++;
          break;
        case 'gco':
          context.globalCompositeOperation = p[i + 1];
          i++;
          break;
        case 'fu':
          p[i + 1](context.canvas.__object__);
          i++;
          break;
        case 'ci':
          context.arc(
  ...
  RGraph.getTextConf = function (args) {
    ...
    var font =
        typeof properties[prefix + 'Font'] === 'string'
          ? properties[prefix + 'Font']
          : properties.textFont,
      size =
        typeof properties[prefix + 'Size'] === 'number'
          ? properties[prefix + 'Size']
          : properties.textSize,
      color =
        typeof properties[prefix + 'Color'] === 'string'
          ? properties[prefix + 'Color']
          : properties.textColor,
      bold = !RGraph.isNullish(properties[prefix + 'Bold'])
        ? properties[prefix + 'Bold']
        : properties.textBold,
      italic = !RGraph.isNullish(properties[prefix + 'Italic'])
        ? properties[prefix + 'Italic']
  ...
  RGraph.responsive = function (conf) {
    ...
    for (var i = 0; i < conf.length; ++i) {
      ...
      // An Initial test
      if (
        document.documentElement.clientWidth >= conf[i].minWidth &&
    ...
    var func = function () {
      ...
      for (var i = 0; i < conf.length; ++i) {
        ...
        if (
          !matched &&
          (document.documentElement.clientWidth <= conf[i].maxWidth ||
            RGraph.isNullish(conf[i].maxWidth))
        ) {
          matched = true;
  ...
  RGraph.drawXAxis = function (obj, opt = {}) {
    var properties = obj.properties,
      context = obj.context,
      tickmarksLength =
        typeof properties.xaxisTickmarksLength === 'number' ? properties.xaxisTickmarksLength : 3,
    ...
    //
    if (
      typeof properties.xaxisLabels === 'object' &&
      !RGraph.isNullish(properties.xaxisLabels) &&
      properties.xaxisLabels.length
    ) {
      for (var i = 0; i < properties.xaxisLabels.length; ++i) {
        if (
          typeof properties.xaxisLabels[i] === 'undefined' ||
          properties.xaxisLabels[i] === null
        ) {
    ...
    if ((obj.type === 'hbar' || obj.type === 'gantt') && properties.xaxisPosition === 'bottom') {
      ...
      (obj.type === 'hbar' || obj.type === 'gantt') &&
      properties.xaxisPosition === 'top'
    ) {
    ...
    if (obj.type === 'drawing.xaxis') {
      if (properties.xaxisPosition === 'center') {
        y =
          (obj.canvas.height - properties.marginTop - properties.marginBottom) / 2 +
    ...
    if (properties.xaxis) {
      ...
      obj.path(
        ...
        obj.canvas.width - properties.marginRight + (isSketch ? 7 : 0),
        y + (isSketch ? 2 : 0),
        ...
        for (var i = 0; i <= xaxisTickmarksCount; ++i) {
          ...
          if (RGraph.isNullish(properties.xaxisTickmarksLastLeft)) {
            if (
              ...
              !(properties.xaxisScaleMax > 0 && properties.xaxisScaleMin < 0)
            ) {
              continue;
            }
          ...
          var x =
            ((obj.canvas.width - properties.marginLeft - properties.marginRight) /
              xaxisTickmarksCount) *
              i +
      ...
      if (properties.xaxisScale) {
        ...
        if (obj.type === 'scatter') {
          ...
          if (properties.xaxisScale) {
            ...
            for (var i = 0; i <= properties.xaxisScaleLabelsCount; ++i) {
              var original =
                ((properties.xaxisScaleMax - properties.xaxisScaleMin) /
                  properties.xaxisScaleLabelsCount) *
                  i +
                properties.xaxisScaleMin;
        ...
        for (var i = 0; i < scale.labels.length; ++i) {
          ...
          var y =
            properties.xaxisPosition === 'top'
              ? obj.marginTop -
                (properties.xaxisTickmarksLength || 0) +
                properties.xaxisLabelsOffsety -
                5
              : obj.canvas.height -
          ...
          if (obj.type === 'drawing.xaxis') {
            if (properties.xaxisPosition === 'top') {
              ...
              y =
                obj.y + (properties.xaxisTickmarksLength || 0) + properties.xaxisLabelsOffsety + 5;
            }
          }
          ...
          RGraph.text({
            ...
            y: y,
            text:
              typeof properties.xaxisScaleFormatter === 'function'
                ? properties.xaxisScaleFormatter({ object: obj, number: scale.values[i] })
                : properties.yaxisPosition === 'right'
                  ? String(scale.labels[scale.labels.length - 1 - i])
                  : String(scale.labels[i]),
            valign:
              typeof properties.xaxisLabelsValign === 'string'
                ? properties.xaxisLabelsValign
                : properties.xaxisPosition === 'top'
                  ? 'bottom'
                  : valign,
            halign:
              typeof properties.xaxisLabelsHalign === 'string'
                ? properties.xaxisLabelsHalign
                : halign,
            marker: false,
            angle: angle,
            tag: 'xaxis.labels'
        ...
        //
        var str =
          (properties.xaxisScaleUnitsPre || '') +
          (properties.xaxisScaleMin || 0)
            .toFixed(properties.xaxisScaleDecimals)
            .replace(/\./, properties.xaxisScalePoint) +
        ...
        RGraph.text({
          ...
          x:
            properties.yaxisPosition === 'right'
              ? obj.canvas.width - properties.marginRight + properties.xaxisLabelsOffsetx
              : properties.yaxisPosition === 'center'
                ? obj.getXCoord(0) + properties.xaxisLabelsOffsetx
                : properties.marginLeft + properties.xaxisLabelsOffsetx,
          y: y,
          text:
            typeof properties.xaxisScaleFormatter === 'function'
              ? properties.xaxisScaleFormatter({
                  object: obj,
                  number: obj.properties.xaxisScaleMin || 0
                })
              : str,
          valign:
            typeof properties.xaxisLabelsValign === 'string'
        ...
        properties.xaxisLabels.length &&
        (properties.xaxisLabelsPosition === 'section' || properties.xaxisLabelsPosition === 'edge')
      ) {
        if (properties.xaxisLabelsPosition === 'edge') {
          var section =
            (obj.canvas.width -
              ...
              (properties.marginInner || 0) -
              (properties.marginInner || 0)) /
            (properties.xaxisLabels.length - 1);
        } else {
          var section =
            (obj.canvas.width - properties.marginLeft - properties.marginRight) /
            properties.xaxisLabels.length;
        ...
        for (var i = 0; i < properties.xaxisLabels.length; ++i) {
          ...
          if (typeof properties.xaxisLabels[i] === 'object' && obj.type === 'scatter') {
            ...
            if (
              properties.xaxisLabels[i + 1] &&
              typeof properties.xaxisLabels[i + 1][1] === 'number'
            ) {
            ...
            var x =
              (obj.getXCoord(rightEdge) - obj.getXCoord(leftEdge)) / 2 + obj.getXCoord(leftEdge);
            ...
            var ret = RGraph.text({
              ...
              text: String(
                RGraph.isNullish(properties.xaxisLabels[i][0]) ? '' : properties.xaxisLabels[i][0]
              ),
              angle: angle,
              marker: false,
              tag: 'labels.specific',
            ...
            obj.path(
              ...
              Math.round(
                properties.marginLeft +
                  ((properties.xaxisLabels[i][1] - properties.xaxisScaleMin) /
                    (properties.xaxisScaleMax - properties.xaxisScaleMin)) *
                    (obj.canvas.width - properties.marginLeft - properties.marginRight)
              ),
              obj.canvas.height - properties.marginBottom,
              Math.round(
                properties.marginLeft +
                  ((properties.xaxisLabels[i][1] - properties.xaxisScaleMin) /
                    (properties.xaxisScaleMax - properties.xaxisScaleMin)) *
              ...
              obj.canvas.height - properties.marginBottom + 20
            );
            ...
            if (i === properties.xaxisLabels.length - 1) {
              obj.path(
                'b m % % l % % s #bbb',
                obj.canvas.width - properties.marginRight,
                obj.canvas.height - properties.marginBottom,
                obj.canvas.width - properties.marginRight,
                obj.canvas.height - properties.marginBottom + 20
              );
            ...
            var y =
              properties.xaxisPosition === 'top'
                ? properties.marginTop + properties.xaxisLabelsOffsety - 5
                : obj.canvas.height - properties.marginBottom + properties.xaxisLabelsOffsety + 5;
            ...
            if (obj.type === 'drawing.xaxis') {
              ...
              if (obj.type === 'drawing.xaxis') {
                if (properties.xaxisPosition === 'top') {
                  y =
                    obj.y - (properties.xaxisTickmarksLength || 0) + properties.xaxisLabelsOffsety;
                } else {
                  y =
                    obj.y +
                    (properties.xaxisTickmarksLength || 0) +
            ...
            var ret = RGraph.text({
              ...
              text: String(
                RGraph.isNullish(properties.xaxisLabels[i]) ? '' : properties.xaxisLabels[i]
              ),
    ...
    if (opt.title !== false) {
      if (properties.xaxisTitle) {
        var x =
          properties.marginLeft +
          (obj.canvas.width - properties.marginLeft - properties.marginRight) / 2 +
          properties.xaxisTitleOffsetx;
        var y =
          properties.xaxisPosition === 'top'
            ? properties.marginTop -
              7 +
              properties.xaxisTitleOffsety -
              properties.xaxisTickmarksLength
            : obj.canvas.height -
              properties.marginBottom +
              7 +
              properties.xaxisTitleOffsety +
        ...
        if (typeof properties.xaxisTitlePos === 'number') {
          if (properties.xaxisPosition === 'top') {
            ...
            y =
              obj.canvas.height -
              properties.marginBottom +
              properties.marginBottom * properties.xaxisTitlePos;
          }
        }
        ...
        RGraph.text({
          ...
          valign:
            properties.xaxisPosition === 'top'
              ? properties.xaxisTitleValign || 'bottom'
              : properties.xaxisTitleValign || 'top',
  ...
  RGraph.drawYAxis = function (obj, opt = {}) {
    var properties = obj.properties,
      ...
      isSketch = obj.type === 'bar' && properties.variant === 'sketch';
    tickmarksLength =
      typeof properties.yaxisTickmarksLength === 'number' ? properties.yaxisTickmarksLength : 3;
    ...
    if (properties.yaxis) {
      ...
      obj.path(
        ...
        x - (isSketch ? 5 : 0),
        properties.marginTop - (isSketch ? 2 : 0),
        x + (isSketch ? 7 : 0),
        obj.canvas.height - properties.marginBottom + (isSketch ? 2 : 0),
      ...
      if (!isSketch) {
        if (properties.yaxisTickmarks) {
          if (typeof properties.yaxisTickmarksCount === 'number') {
            ...
            var yaxisTickmarksCount = properties.yaxisLabelsSpecific
              ? properties.yaxisLabelsSpecific.length - 1
              : properties.yaxisLabelsCount;
        ...
        for (var i = 0; i <= yaxisTickmarksCount; ++i) {
          ...
          if (RGraph.isNullish(properties.yaxisTickmarksLastBottom)) {
            if (
              i === yaxisTickmarksCount &&
              properties.xaxis &&
              properties.xaxisPosition === 'bottom'
            ) {
              continue;
            }
          ...
          var y =
            ((obj.canvas.height - properties.marginTop - properties.marginBottom) /
              yaxisTickmarksCount) *
              i +
            properties.marginTop;
    ...
    if (opt.labels !== false) {
      ...
      if (properties.yaxisScale && !properties.yaxisLabelsSpecific) {
        if (obj.type === 'drawing.yaxis') {
          if (properties.yaxisScale) {
            ...
            for (var i = 0; i <= properties.yaxisScaleLabelsCount; ++i) {
              var original =
                ((properties.yaxisScaleMax - properties.yaxisScaleMin) /
                  properties.yaxisScaleLabelsCount) *
                  i +
                properties.yaxisScaleMin;
              var hmargin = properties.marginInner;
        ...
        if (properties.xaxisPosition === 'center') {
          ...
          for (var i = 0; i < scale.labels.length; ++i) {
            ...
            RGraph.text({
              ...
              x:
                properties.yaxisPosition === 'right'
                  ? x + (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx + 5
                  : x - (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx - 5,
              y: properties.yaxisScaleInvert
                ? halfHeight + properties.marginTop - section * i + properties.yaxisLabelsOffsety
                : properties.marginTop + section * i + properties.yaxisLabelsOffsety,
              text: String(scale.labels[scale.labels.length - 1 - i]),
              valign:
                typeof properties.yaxisLabelsValign === 'string'
                  ? properties.yaxisLabelsValign
                  : valign,
              halign:
                typeof properties.yaxisLabelsHalign === 'string'
                  ? properties.yaxisLabelsHalign
                  : properties.yaxisPosition === 'right'
                    ? 'left'
                    : halign,
              marker: false,
            ...
            RGraph.text({
              ...
              x:
                properties.yaxisPosition === 'right'
                  ...
                  : x - (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx - 5,
              y: properties.yaxisScaleInvert
                ? halfHeight + properties.marginTop + section * i
                : y + properties.yaxisLabelsOffsety,
              text:
                '-' +
                (properties.yaxisScaleInvert
                  ? String(scale.labels[scale.labels.length - i - 1])
                  : String(scale.labels[i])),
              valign:
                typeof properties.yaxisLabelsValign === 'string'
                  ? properties.yaxisLabelsValign
          ...
          RGraph.text({
            ...
            x:
              properties.yaxisPosition === 'right'
                ? x + 5 + (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx
                : x - 5 - (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx,
            y: properties.yaxisScaleInvert
              ? properties.marginTop + properties.yaxisLabelsOffsety
              : properties.marginTop +
                (obj.canvas.height - properties.marginBottom - properties.marginTop) / 2 +
                properties.yaxisLabelsOffsety,
            text:
              typeof properties.yaxisScaleFormatter === 'function'
                ? properties.yaxisScaleFormatter({
                    object: this,
                    number: 0,
                    unitspre: properties.yaxisScaleUnitsPre,
                    unitspost: properties.yaxisScaleUnitsPost,
                ...
                : (properties.yaxisScaleUnitsPre || '') +
                  properties.yaxisScaleMin
                    .toFixed(properties.yaxisScaleDecimals)
                    .replace(/\./, properties.yaxisScalePoint) +
                  (properties.yaxisScaleUnitsPost || ''),
            valign:
              typeof properties.yaxisLabelsValign === 'string'
                ? properties.yaxisLabelsValign
                : valign,
          ...
          RGraph.text({
            ...
            x:
              properties.yaxisPosition === 'right'
                ...
                : x - 5 - (properties.yaxisTickmarksLength || 0) + properties.yaxisLabelsOffsetx,
            y: properties.yaxisScaleInvert
              ? obj.canvas.height - properties.marginBottom + properties.yaxisLabelsOffsety
              : properties.marginTop + properties.yaxisLabelsOffsety,
            text:
              typeof properties.yaxisScaleFormatter === 'function'
                ? properties.yaxisScaleFormatter({
          ...
          for (var i = 0; i < scale.labels.length; ++i) {
            ...
            RGraph.text({
              ...
              text: String(
                properties.yaxisScaleInvert
                  ? scale.labels[i]
                  : scale.labels[scale.labels.length - 1 - i]
              ),
              valign:
                typeof properties.yaxisLabelsValign === 'string'
                  ? properties.yaxisLabelsValign
        ...
        properties.yaxisLabelsSpecific.length &&
        (properties.yaxisLabelsPosition === 'section' || properties.yaxisLabelsPosition === 'edge')
      ) {
        var section =
          (obj.canvas.height - properties.marginTop - properties.marginBottom) /
          (properties.yaxisLabelsSpecific.length -
            (properties.yaxisLabelsPosition === 'section' ? 0 : 1));
        ...
        for (var i = 0; i < properties.yaxisLabelsSpecific.length; ++i) {
          var y =
            properties.marginTop +
            section * i +
            (properties.yaxisLabelsPosition === 'section' ? section / 2 : 0);

          var ret = RGraph.text({
            object: obj,
            textConfPrefix: 'yaxisLabels',
            x:
              obj.type === 'drawing.yaxis'
                ? properties.yaxisPosition === 'right'
                  ? obj.x + 7 + properties.yaxisLabelsOffsetx
                  : obj.x - 5 + properties.yaxisLabelsOffsetx
                : properties.yaxisPosition === 'right'
                  ? x + properties.yaxisLabelsOffsetx + 5
                  : properties.marginLeft - 5 + properties.yaxisLabelsOffsetx,
            y: y + properties.yaxisLabelsOffsety,
    ...
    if (opt.title) {
      if (properties.yaxisTitle) {
        ...
        if (
          ...
          maxLabelLength = (function (labels) {
            ...
            for (var i = 0, max = 0; i < labels.length; ++i) {
              var dim = RGraph.measureText(labels[i], textConf.bold, textConf.font, textConf.size);
        ...
        var x =
          properties.yaxisPosition === 'right'
            ? obj.canvas.width - properties.marginRight + 5 + maxLabelLength + 10
            : properties.marginLeft - 5 - maxLabelLength - 10;
        var y =
          (obj.canvas.height - properties.marginTop - properties.marginBottom) / 2 +
          properties.marginTop;
        ...
        RGraph.text({
          ...
          marker: false,
          accessible:
            typeof properties.yaxisTitleAccessible === 'boolean'
              ? properties.yaxisTitleAccessible
              : undefined,
          angle: -45,
  ...
  RGraph.getLabelsCSSClassName = function () {
    ...
    if (typeof properties[args.name] === 'string') {
      ...
      if (
        typeof properties[args.name] === 'object' &&
        typeof properties[args.name][args.index] === 'string'
      ) {
  ...
  RGraph.clipTo.start = function () {
    ...
    if (RGraph.isString(args.dimensions)) {
      if (args.dimensions === 'lefthalf') {
        ...
        args.object.path('sa b r % % % % cl', x, y, width, height);
        ...
        // object
        if (
          !RGraph.isNumber(args.object.centerx) ||
          !RGraph.isNumber(args.object.centery) ||
          !RGraph.isNumber(args.object.radius)
        ) {
          alert(
            '[RGRAPH CLIPPING] To use the r: syntax the object (Type: {1}, ID: {2}, UID: {3}) must support the centerx, centery and radius properties.'.format(
              args.object.type,
              args.object.id,
              args.object.uid
            )
        ...
        args.dimensions.match(
          /^(?:segment|arc): *([-.0-9degrad]+) *, *([-.0-9degrad]+) *, *([-.0-9degrad]+) *, *([-.0-9degrad]+) *, *([-.0-9degrad]+)$/i
        )
      ) {
        ...
        if (args.object.clipToScaleWorker) {
          ...
          console.log(
            "The scale: clipping option isn't implemented for this chart type (" +
              args.object.type +
              ')'
          );
        }
      ...
      for (var i = 0, path = []; i < args.dimensions.length; ++i) {
        path.push(
          '%1 %2 %3'.format(i === 0 ? 'm' : 'l', args.dimensions[i][0], args.dimensions[i][1])
        );
      }
  ...
  RGraph.clipTo.test = function () {
    ...
    if (args.object.properties.clip) {
      ...
      if (clip === 'lefthalf') {
        ...
        args.object.path('b r % % % %', x, y, width, height);
    ...
    if (lines) {
      ...
      for (let i = 0; i < obj.properties.horizontalLines.length; ++i) {
        var conf = lines[i],
          ...
          textItalic =
            typeof conf.labelItalic === 'boolean' ? conf.labelItalic : obj.properties.textItalic;
        ...
        if (
          conf.dashed === true ||
          (typeof conf.dashed === 'undefined' && typeof conf.dotted === 'undefined')
        ) {
          linedash = '[6,6]';
        }
        ...
        if (['bar', 'line', 'scatter'].includes(obj.type)) {
          ...
          if (typeof conf.labelPosition === 'string' && conf.labelPosition.indexOf('left') >= 0) {
            ...
            typeof conf.labelPosition === 'string' &&
            conf.labelPosition.indexOf('center') >= 0
          ) {
            textX =
              (obj.canvas.width - obj.properties.marginLeft - obj.properties.marginRight) / 2 +
              obj.properties.marginLeft;
            ...
            typeof conf.labelPosition === 'string' &&
            conf.labelPosition === 'right-margin'
          ) {
            textX = obj.canvas.width - obj.properties.marginRight + hmargin;
            halign = 'left';
          } else if (
            typeof conf.labelPosition === 'string' &&
            conf.labelPosition === 'left-margin'
          ) {
        ...
        var num = RGraph.numberFormat({
          object: obj,
          number: (typeof conf.value === 'number' ? conf.value : avg).toFixed(
            typeof conf.labelValueDecimals === 'number'
              ? conf.labelValueDecimals
              : defaults.labelValueDecimals
          ),
        ...
        RGraph.text({
          object: obj,
          text: (typeof conf.label === 'string' ? conf.label : defaults.label).replace(
            '%{value}',
            num
          ),
          ...
          y:
            ...
            linewidth / 2 +
            parseFloat(
              (conf.labelPosition.indexOf('top') !== -1
                ? -1 * conf.labelOffsety
                : conf.labelOffsety) || defaults.labelOffsety
            ),
          valign: valign,
          halign: halign,
          size: textSize,
  ...
  RGraph.tooltipsHotspotIgnore = function (obj, index) {
    if (
      (RGraph.isBoolean(obj.properties.tooltipsHotspotIgnore) &&
        obj.properties.tooltipsHotspotIgnore) ||
      (RGraph.isNumber(obj.properties.tooltipsHotspotIgnore) &&
        obj.properties.tooltipsHotspotIgnore === index) ||
      (RGraph.isArray(obj.properties.tooltipsHotspotIgnore) &&
        obj.properties.tooltipsHotspotIgnore.includes(index)) ||
      (RGraph.isArray(obj.properties.tooltipsHotspotIgnore) &&
        obj.properties.tooltipsHotspotIgnore[index] === true)
    ) {
      return true;
    }
  ...
  RGraph.isTextual = function (obj) {
    return typeof obj === 'string' || typeof obj === 'number';
  };
  RGraph.isNumeric = function (value) {
    value = String(value);
    return (
      Boolean(value.match(/^[-.0-9]+$/)) ||
      Boolean(value.match(/^[-.0-9]+e[-0-9]+$/)) ||
      value === 'Infinity' ||
      value === '-Infinity' ||
      Boolean(value.match(/^[-.0-9]+x[0-9a-f]+$/i))
    );
  };
  ...
  RGraph.isObject = function (obj) {
    return obj &&
      typeof obj === 'object' &&
      obj.constructor.toString().toLowerCase().indexOf('object') > 0
      ? true
      : false;
  };
...
window.$d = function (m) {
  ...
  if (!this.rgraph_debug_textarea) {
    ...
    if (window.localStorage.rgraph_debug_textarea_w)
    ...
    window.addEventListener(
      ...
      function (e) {
        if (this.rgraph_debug_textarea.mousedown) {
          window.localStorage.rgraph_debug_textarea_w =
            this.rgraph_debug_textarea.offsetWidth + 'px';
          window.localStorage.rgraph_debug_textarea_h =
            this.rgraph_debug_textarea.offsetHeight + 'px';
        }
      },
      false
    );
