...
RGraph.Line = function (conf) {
  ...
  this.draw = function () {
    ...
    if (
      properties.filled &&
      !properties.filledRange &&
      this.data.length > 1 &&
      properties.filledAccumulative
    ) {
      var accumulation = [];
      ...
      for (var set = 0; set < this.data.length; ++set) {
        for (var point = 0; point < this.data[set].length; ++point) {
          this.data[set][point] =
            Number(accumulation[point] ? accumulation[point] : 0) + this.data[set][point];
    ...
    if (properties.yaxisScaleMax) {
      ...
      for (dataset = 0; dataset < this.data.length; ++dataset) {
        for (var datapoint = 0; datapoint < this.data[dataset].length; datapoint++) {
          this.max = Math.max(
            this.max,
            this.data[dataset][datapoint] ? Math.abs(parseFloat(this.data[dataset][datapoint])) : 0
          );
    ...
    for (var i = 0, j = 0, len = this.data.length; i < len; i++, j++) {
      ...
      if (properties.filledColors) {
        if (typeof properties.filledColors == 'object' && properties.filledColors[j]) {
          ...
          typeof properties.filledColors == 'object' &&
          properties.filledColors.toString().indexOf('Gradient') > 0
        ) {
        ...
        for (var i = 0, len = this.coords2.length; i < len; ++i) {
          ...
          for (var j = 0; j < this.coords2[i].length; ++j) {
            if (
              ...
              var tickmarks =
                typeof properties.tickmarksStyle == 'object'
                  ? properties.tickmarksStyle[i]
                  : properties.tickmarksStyle;
      ...
      for (var i = 0, len = this.coords2.length; i < len; i += 1) {
        for (var j = 0, len2 = this.coords2[i].length; j < len2; ++j) {
          if (
            ...
            var tickmarks =
              typeof properties.tickmarksStyle == 'object' &&
              !RGraph.isNullish(properties.tickmarksStyle)
                ? properties.tickmarksStyle[i]
                : properties.tickmarksStyle;
            this.context.strokeStyle = properties.colors[i];
            this.drawTick(
    ...
    for (var i = 0; i < this.data.length; ++i) {
      if (
        (RGraph.isArray(properties.trendline) && properties.trendline[i]) ||
  ...
  this.drawLine = function (lineData, color, fill, linewidth, tickmarks, index) {
    ...
    var xInterval =
      (this.canvas.width - 2 * properties.marginInner - this.marginLeft - this.marginRight) /
      (lineData.length - 1);
    ...
    for (i = 0, len = lineData.length; i < len; i += 1) {
      ...
      if (
        ...
        (properties.xaxisPosition == 'bottom' &&
          lineData[i] < this.min &&
          !properties.outofbounds) ||
        (properties.xaxisPosition == 'center' &&
          lineData[i] < -1 * this.max &&
          !properties.outofbounds) ||
        (((lineData[i] < this.min && properties.xaxisPosition !== 'center') ||
          lineData[i] > this.max) &&
          !properties.outofbounds)
      ) {
        yPos = null;
    ...
    for (var i = 0, len = lineCoords.length; i < len; i += 1) {
      ...
      if (
        !properties.outofbounds &&
        (prevY < this.marginTop || prevY > this.canvas.height - this.marginBottom)
      ) {
        penUp = true;
      }
      ...
      if (i == 0 || penUp || !yPos || !prevY || prevY < this.marginTop) {
        ...
        if (
          (yPos >= this.marginTop && yPos <= this.canvas.height - this.marginBottom) ||
          properties.outofbounds
        ) {
          if (
            isLast &&
    ...
    if (properties.filled && !properties.filledRange && !properties.spline) {
      ...
      if (index > 0 && properties.filledAccumulative) {
        ...
        for (var k = i - 1; k >= 0; --k) {
          this.context.lineTo(
            k == 0 ? prevLineCoords[k][0] + 1 : prevLineCoords[k][0],
            prevLineCoords[k][1]
          );
        }
      } else {
  ...
  this.drawTick = function (
    ...
    if (
      ...
      if (
        ...
        tickmarks == 'circle' ||
        tickmarks == 'filledcircle' ||
        ((tickmarks == 'endcircle' || tickmarks === 'filledendcircle') &&
        ...
        if (tickmarks.indexOf('filled') !== -1) {
          this.path('fs %', isShadow ? properties.shadowColor : color);
        } else {
          this.path('fs %', isShadow ? properties.shadowColor : 'white');
        }
      ...
      tickmarks == 'triangle' ||
      tickmarks == 'filledtriangle' ||
      (tickmarks == 'endtriangle' && (index == 0 || index == lineData.length - 1))
    ) {
      this.path(
        'b m % % l % % l % % c f % s null',
  ...
  this.drawRange = function () {
    ...
    if (properties.filledRange && properties.filled) {
      ...
      for (var idx = 0; idx < 2; ++idx) {
        ...
        this.context.fillStyle =
          idx == 1
            ? properties.filledRangeThresholdColors[1]
            : properties.filledRangeThresholdColors[0];
  ...
  this.redrawLine = function (coords, color, linewidth, index) {
    ...
    this.context.strokeStyle =
      typeof color == 'object' && color && color.toString().indexOf('CanvasGradient') == -1
        ? color[0]
        : color;
  ...
  this.drawBackdrop = function (coords, color) {
    ...
    this.path('lw % ga % ss %', size, properties.backdropAlpha, color);
  ...
  this.getLineWidth = function (i) {
    ...
    if (typeof linewidth == 'number') {
      ...
      alert(
        '[LINE] Error! The linewidth option should be a single number or an array of one or more numbers'
      );
    }
  };
    ...
    for (var i = 0; i < obj.coords.length; ++i) {
      ...
      if (
        ...
        return {
          ...
          sequentialIndex: i,
          label:
            properties.xaxisLabels && typeof properties.xaxisLabels[idx] === 'string'
              ? properties.xaxisLabels[idx]
              : null,
          tooltip: typeof tooltip === 'string' ? tooltip : null
        };
      } else if (
        properties.tooltipsHotspotXonly == true &&
        mouseX <= x + properties.tooltipsHotspotSize &&
        mouseX >= x - properties.tooltipsHotspotSize
      ) {
        var tooltip = RGraph.parseTooltipText(properties.tooltips, i);
          ...
          specific && specific[i]
            ...
            : specific
              ...
              : RGraph.numberFormat({
                  object: this,
                  number:
                    typeof decimals === 'number' ? data_arr[i].toFixed(decimals) : data_arr[i],
  ...
  this.getValue = function (arg) {
    ...
    if (properties.xaxisPosition == 'center') {
      var value =
        ((obj.grapharea / 2 - (mouseY - properties.marginTop)) / obj.grapharea) *
        (obj.max - obj.min);
      value *= 2;
      ...
      var value =
        ((obj.grapharea - (mouseY - properties.marginTop)) / obj.grapharea) * (obj.max - obj.min);
    ...
    if (properties.tooltipsHighlight) {
      if (typeof properties.highlightStyle === 'function') {
        ...
        this.path(
          ...
          properties.marginTop,
          this.canvas.width - properties.marginRight,
          this.canvas.height - properties.marginBottom,
          properties.marginLeft,
          this.canvas.height - properties.marginBottom,
          properties.marginLeft,
          properties.marginTop,
  ...
  this.adjusting_mousemove = function (e) {
    ...
    if (
      ...
      RGraph.Registry.get('adjusting') &&
      RGraph.Registry.get('adjusting').uid == this.uid
    ) {
  ...
  this.drawSpline = function (context, coords, color, index) {
    ...
    var interval =
      (this.canvas.width - (marginLeft + marginRight) - 2 * hmargin) / (coords.length - 1);
  ...
  this.parseColors = function () {
    ...
    for (var i = 0; i < properties.colors.length; ++i) {
      if (
        typeof properties.colors[i] == 'object' &&
        properties.colors[i][0] &&
        properties.colors[i][1]
      ) {
    ...
    if (properties.filledColors) {
      if (typeof properties.filledColors == 'string') {
        properties.filledColors = this.parseSingleColorForGradient(
          properties.filledColors,
          'vertical'
        );
      } else {
        for (var i = 0; i < properties.filledColors.length; ++i) {
          properties.filledColors[i] = this.parseSingleColorForGradient(
            properties.filledColors[i],
            'vertical'
          );
        }
      }
    ...
    if (!RGraph.isNullish(properties.backgroundHbars)) {
      for (var i = 0; i < properties.backgroundHbars.length; ++i) {
        properties.backgroundHbars[i][2] = this.parseSingleColorForGradient(
          properties.backgroundHbars[i][2]
        );
      }
    }
        ...
        if (obj.original_data[i].length) {
          for (var j = 0, len2 = obj.original_data[i].length; j < len2; ++j) {
            ...
            if (dataset[j] > center_value) {
              dataset[j] =
                original_data[i][j] - (original_data[i][j] - center_value) * (frame / frames);
            } else {
              dataset[j] =
                original_data[i][j] + ((center_value - original_data[i][j]) / frames) * frame;
            }
          }
        }
      }
    ...
    for (var i = 0, len = obj.original_data.length; i < len; ++i) {
      for (var j = 0; j < obj.original_data[i].length; ++j) {
        obj.original_data[i][j] =
          obj.get('filled') && obj.get('filledAccumulative') && i > 0 ? 0 : half;
      }
    }
  ...
  this.isAdjustable = function (shape) {
    ...
    if (
      RGraph.isArray(properties.adjustableOnly) &&
      properties.adjustableOnly[shape.sequentialIndex]
    ) {
      return true;
    }
    ...
    this.datasetTooltipsListener = function (e) {
      ...
      if (RGraph.isObject(result) && RGraph.isNumber(result.dataset)) {
        ...
        if (
          e.type === 'click' ||
          (e.type === 'mousemove' &&
            obj.properties.tooltipsDatasetEvent === 'mousemove' &&
            (!RGraph.Registry.get('tooltip') || i != RGraph.Registry.get('tooltip').__dataset__))
        ) {
          RGraph.hideTooltip();
          RGraph.redraw();
  ...
  this.drawTrendline = function () {
    ...
    for (
      var i = 0,
        ...
        yCoordMinusAverageY = [],
        valuesMultiplied = [],
        xCoordMinusAverageSquared = [];
      i < this.data[args.dataset].length;
      ++i
    ) {
    ...
    if (
      properties.trendlineDashed === true ||
    ...
    if (
      properties.trendlineDotted === true ||
    ...
    if (RGraph.isArray(properties.trendlineDashArray)) {
      if (
        properties.trendlineDashArray.length === 2 &&
        ...
        RGraph.isArray(properties.trendlineDashArray) &&
    ...
    this.path(
      ...
      (x2 = Math.min(
        coords[1][0],
        this.coords2[args.dataset][this.coords2[args.dataset].length - 1][0] + margin
      )),
      (y2 = coords[1][1]),
      // stroke color
      color
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (RGraph.isNullish(data[i]) && readData) {
        ...
        for (var j = i + 1; j < data.length; ++j) {
          ...
          this.path(
            'b lw % m % % l % % s %',
            typeof properties.nullBridgeLinewidth === 'number'
              ? properties.nullBridgeLinewidth
              : this.getLineWidth(datasetIdx),
            this.coords2[datasetIdx][start][0],
            this.coords2[datasetIdx][start][1],
            this.coords2[datasetIdx][end][0],
            this.coords2[datasetIdx][end][1],
            typeof properties.nullBridgeColors === 'string'
              ? properties.nullBridgeColors
              : typeof properties.nullBridgeColors === 'object' &&
                  !RGraph.isNullish(properties.nullBridgeColors) &&
                  properties.nullBridgeColors[datasetIdx]
                ? properties.nullBridgeColors[datasetIdx]
                : properties.colors[datasetIdx]
          );
  ...
  this.drawAngledLabels = function () {
    if (properties.labelsAngled) {
      ...
      var getTextConfiguration = function (dir) {
        ...
        for (var prefix in prefixes) {
          for (var prop in textProperties) {
            ...
            if (name) {
              if (
                RGraph.isString(properties[name]) ||
      ...
      for (var i = 0; i < coords.length - 1; ++i) {
        ...
        RGraph.text({
          object: this,
          accessible: properties.adjustable
            ? false
            : RGraph.isBoolean(properties.labelsAngledAccessible)
          ...
          y: coords[i][1] + dy - 5,
          text:
            properties.labelsAngledSpecific &&
            (RGraph.isString(properties.labelsAngledSpecific[i]) ||
              RGraph.isNumber(properties.labelsAngledSpecific[i]))
              ? properties.labelsAngledSpecific[i]
              : properties.labelsAngled[direction],
          halign: 'center',
          valign: 'bottom',
  ...
  this.growPoint = function () {
    ...
    for (var i = 0; i < frames; i++) {
      (function (i) {
        setTimeout(function () {
          obj.original_data[args.dataset][args.index] =
            ((args.value - original_value) * (i + 1)) / frames + original_value;
  ...
  this.getKeyValue = function (index) {
    if (
      RGraph.isArray(this.properties.keyFormattedValueSpecific) &&
  ...
  this.over = function (x, y) {
    ...
    for (var i = 0; i < datasets.length; ++i) {
      ...
      for (var j = 1; j < datasets[dataset].length; ++j) {
        this.context.lineTo(datasets[dataset][j][0], datasets[dataset][j][1]);
      }
      ...
      //
      if (this.properties.filled && this.properties.filledAccumulative) {
      ...
      } else if (this.properties.filled && !this.properties.filledAccumulative) {
      ...
      if (
        this.context.isPointInStroke(args.x, args.y) ||
  ...
  this.highlightDataset = function () {
    ...
    if (
      this.properties.highlightDatasetStrokeUseColors &&
      this.properties.colors[args.opt.dataset]
    ) {
    ...
    if (
      this.properties.highlightDatasetFillUseColors &&
      RGraph.isArray(this.properties.filledColors) &&
      this.properties.filledColors[args.opt.dataset]
    ) {
    ...
    // to the path
    //
    if (this.properties.filled && this.properties.filledAccumulative && args.opt.dataset > 0) {
      // Necessary?
      this.context.lineTo(
      ...
      for (let i = coords[args.opt.dataset - 1].length - 1; i >= 0; --i) {
        this.context.lineTo(coords[args.opt.dataset - 1][i][0], coords[args.opt.dataset - 1][i][1]);
      }
    }
  ...
  this.clipToScaleWorker = function (clip) {
    ...
    // library
    if (RegExp.$1 === 'min') from = this.scale2.min;
    else from = Number(RegExp.$1);
    if (RegExp.$2 === 'max') to = this.scale2.max;
    else to = Number(RegExp.$2);
    ...
    this.path('sa b r % % % % cl', x, y, width, height);
  };
  ...
  this.clipToScaleTestWorker = function (clip) {
    ...
    this.path('b r % % % %', x, y, width, height);
  };
...
RGraph.Line.dualColorTrendline = function (args) {
  ...
  if (
    RGraph.isString(args.animationEffect) &&
